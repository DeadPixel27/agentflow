"""Supabase persistence for users, runs, and workflows."""
...
from app.persistence.supabase_client import get_supabase
...
def save_run(run: RunResult) -> None:
    ...
    client.table("workflow_runs").upsert(
        {
            "id": run.run_id,
    ...
    client.table("workflow_step_runs").delete().eq("run_id", run.run_id).execute()
    ...
    if step_rows:
        client.table("workflow_step_runs").insert(step_rows).execute()
...
def get_run(run_id: str) -> Optional[RunResult]:
    ...
    run_resp = (
        client.table("workflow_runs").select("*").eq("id", run_id).maybe_single().execute()
    )
    ...
    row = run_resp.data
    steps_resp = (
        client.table("workflow_step_runs")
        .select("*")
        .eq("run_id", run_id)
        .order("step_order")
        .execute()
...
def save_workflow(workflow: WorkflowRecord) -> None:
    ...
    client.table("workflow_steps").delete().eq("workflow_id", workflow.workflow_id).execute()
    ...
    if step_rows:
        client.table("workflow_steps").insert(step_rows).execute()
...
def list_workflows(user_id: Optional[str] = None) -> list[WorkflowSummary]:
    ...
    query = client.table("workflows").select("*").order("created_at", desc=True)
    if user_id is not None:
        query = query.eq("user_id", user_id)
    resp = query.execute()

    steps_resp = client.table("workflow_steps").select("workflow_id").execute()
